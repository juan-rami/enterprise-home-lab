...
4. Opened Server Manager and clciked roles and Features and selected role-based installation to check Active Directory Domain Services and install it. A notification flag came up, and selected Promote this server to a domain controller, chose Add a new forest, and entered the company.local domain and set the password.
5.  The administrator account was created and was successfully able to log in with the password that I set up
